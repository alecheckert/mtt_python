...
In brief, detection of spots is accomplished with a generalized
log-likelihood ratio test based on the assumption of a Gaussian PSF
and Gaussian background noise. Detections are then subjected to
Gauss-Newton fitting (Gaussian PSF) and recorded. Next, 
trajectories are generated by considering the relationship between
existing trajectories and new localizations as a semigraph problem.
The semigraph adjacency matrix is reduced to independent connected
components, each of which represents a (possibly ambiguous) association
between 1 or more trajectories and 1 or more localizations. The 
assignments between trajectories and localizations are then permuted
to maximize log-likelihood under a Brownian diffusion model.
Finally, trajectories are recorded in a MAT file.
...
def localize(
	...
	g = gausswin2(psf_std, window_size)
	gc = g - g.sum() / T
	Sgc2 = (gc**2).sum()
	hgc = expand_w(gc, N, M)
	...
	detect_masque = carte_MV > pfa

	#record results
	result = []
	if detect_masque.sum() == 0:
		liste_detect = np.zeros(6)
		detect_pfa = np.zeros(detect_masque.shape)
	else:
		detect_pfa = all_max_2d(carte_MV).astype('bool') & detect_masque
		...
		liste_detect = np.zeros((n_detect, 7))
		liste_detect[:,0] = np.arange(n_detect)
		liste_detect[:,1] = di
		liste_detect[:,2] = dj
		liste_detect[:,3] = alpha_detect
		liste_detect[:,4] = sig2_detect
		liste_detect[:,5] = psf_std
		liste_detect[:,6] = np.ones(n_detect)
		...
		for i in range(n_detect):
			test_bord = (liste_detect[i,1] < bord) | (liste_detect[i,1] > N - bord) | \
				(liste_detect[i,2] < bord) | (liste_detect[i,2] > M - bord)
			if liste_detect[i,3] > 0.0 and not test_bord:
				y_coord, x_coord, alpha, Sig2, offset, r, result_ok, naive_var = fitGaussian(
					image_2d,
					int(liste_detect[i,1]),
					int(liste_detect[i,2]),
					psf_std
				)
				result.append((y_coord, x_coord, alpha, Sig2, offset, r, result_ok, naive_var))

	return result, liste_detect
...
def fitGaussian(
	...
	im_part = image[Pi - half_window + 1: Pi + half_window + 2, Pj - half_window + 1: Pj + half_window + 2]
	prec_rel = 0.01
	...
	def fitGaussianIter(
		...
		derder_g_i0 = (-1 + iiii/(r0**2)) * g_div_sq_r0
		derder_g_j0 = (-1 + jjjj/(r0**2)) * g_div_sq_r0

		der_J_i0 = alpha * (der_g_i0 * err).sum()
		der_J_j0 = alpha * (der_g_j0 * err).sum()

		derder_J_i0 = alpha * (derder_g_i0 * err).sum() - (alpha**2) * (der_g_i0**2).sum()
		derder_J_j0 = alpha * (derder_g_j0 * err).sum() - (alpha**2) * (der_g_j0**2).sum()
	...
	bornes_ijr = [-1.5, 1.5, -1.5, 1.5, psf_std - 50 * psf_std / 100, psf_std + 50 * psf_std / 100]
	test = True 
	r = psf_std
	i = 0.0
	j = 0.0
	...
	while test:
		...
		if cpt > ITER_MAX:
			test = False
		result_ok = not ((i < bornes_ijr[0]) or (i > bornes_ijr[1]) \
			or (j < bornes_ijr[2]) or (j > bornes_ijr[3]) or (r < bornes_ijr[4]) \
			or (r > bornes_ijr[5]))
