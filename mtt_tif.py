'''
mtt_tif.py -- implementation of the multiple target tracing algorithm 
to localize and track fluorophores in single molecule microscopy
data.

Updated for TIF files.
...
def localizeAndTrackND2file(
	...
	if '.tiff' in tif_file:
		os.system('mv %s %s' % (tif_file, tif_file.replace('.tiff', '.tif')))
		tif_file = tif_file.replace('.tiff', '.tif')
	f = tifffile.imread(tif_file)

	#sometimes necessary for TIF files originating from BioFormats
	if len(f.shape) == 5:
		if (f.shape[0] == 1) and (f.shape[2] == 1):
			f = f[0,:,0,:,:]
		else:
			print('mtt_tif.py not implemented for TIFF files of this type. Ask Alec. He can fix it.')
			exit(1)
	elif len(f.shape) == 3:
		pass
	else:
	...
	loc_file = tif_file.replace('.tif', '_locs.txt')
	if not mat_save_file:
		mat_save_file = tif_file.replace('.tif', '_Tracked.mat')
	time_0 = time.clock()
	localizeTIFfile(
		tif_file,
		outfile = loc_file,
...
def localizeTIFfile(
	...
	time_0 = time.clock()
	f = tifffile.imread(tif_file)
	if (len(f.shape) == 5) and (f.shape[0] == 1) and (f.shape[2] == 1):
		f = f[0,:,0,:,:]
	elif (len(f.shape) == 3):
		pass
	else:
		print('mtt_tif.py currently not implemented for TIF files of this type. Ask Alec. He can fix it.')
		exit(1)

	N_frames = f.shape[0]
	if not outfile:
		outfile = tif_file.replace('.tif', '_locs.txt')
	...
	print('Finished localizing %s (%.1f sec)' % (tif_file, time.clock() - time_0))
...
if __name__ == '__main__':
	...
	parser.add_argument(
		'tif_file',
		type = str,
		help = 'tif file or directory with tif files'
	)
	parser.add_argument(
		'-o',
		'--out_mat_file',
	...
	if os.path.isdir(args.tif_file):
		path_list = ['%s/%s' % (args.tif_file, i) for i in os.listdir(args.tif_file) if ('.tif' in i) or ('.tiff' in i)]
		for tif_path in path_list:
	...
	elif os.path.isfile(args.tif_file):
		localizeAndTrackND2file(
			args.tif_file,
			error_rate = args.error_rate,
			pixel_size_um = args.pixel_size_um,
